...
dashboard-admin-deployment:
  cmd.run:
    - name: {{ pillar['kubernetes']['binary-root'] }}/server/bin/kubectl --kubeconfig={{ pillar['kubernetes']['conf-root'] }}/admin.kubeconfig create -f {{ pillar['kubernetes']['conf-root'] }}/deploys/dashboard-admin.yaml
    - unless: kubectl --kubeconfig={{ pillar['kubernetes']['conf-root'] }}/admin.kubeconfig get ClusterRoleBinding | grep kubernetes-dashboard
